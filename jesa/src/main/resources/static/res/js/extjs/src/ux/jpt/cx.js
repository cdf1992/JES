						...
						if (jsonData === undefined) {
							...
							/*} else if (key1 === "01") {*/
								...
								if (key5 !== "") {
									...
									var skssn = infoArrSsq[0].substring(0, 4); // 税款所属日期对应的年份
									  ssqn = skssn;
			                          ssqy = month;
        ...
        for (var i = 1; i <= month; i++) {
            ...
            for (var j = 0; j < data.length && j <= month; j++) {
                ...
                if (tmpMonth == temp_yf) {
                    ...
                    if (temp[0] == (ssq_nf + "" + ssq_yf)) { //如果是当前税款所属日期，判断是否已申报、回退税款所属日期、 注销/撤销勾选

                        var token = getCookie("token");
                        var objs = token.split(MENUSPLIT);
                        if (temp.length == 7 && temp[5] == 'true') {
                            if (objs[9] == '0') {//isHt为0，允许回退税款所属日期 20180510 start
                               // $('#ht_yf_' + temp_yf).show();
                            }
                        }
						...
						success: function(e) {
							showMsg("请求已响应！");
								...
								if("01"==t){
									...
									showMsg("查询您的当前税款所属日期出现异常，请稍后再试！");
								}else{
									showMsg("返回值为："+t+"未知问题");
								}
				    },
               ...
               $.ajax({
                   ...
                   success: function(jsonData) {
                       ...
                       if(key1=="200"){
                          ...
                          /* //切换税款所属日期
                           isQh = token.split(MENUSPLIT)[10];//按季切换税款所属日期标志
                           if(isQh=="0"){//可以切换
                               $("#qhskssq").show();
                           }else{
                               $("#qhskssq").hide();
		...
		if(isCanPost) {
           ...
           // spinner.spin(target);
		    ...
		    cxqhssq(cert, token,param);
			 ...
			 searchCallBack("error","当前不允许切换税款所属日期！");
        }
	}
}
