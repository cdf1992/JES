		...
		handler : function() {
			...
			Ext.MessageBox.confirm("提示", "是否仅导出勾选的数据", function(obj) {
				...
				if (dataGrid.length > 0) {
				    ...
				    downForm.action = "downLoadCheckExcel.do"; // downLoadCheckExcel.ajax
...
//导出
function exportExcelSubmit(params,url){
	...
	buttons : [ {
	    ...
	    handler : function(me) {
	    	if(dataParamList.length>0){
				...
				exportExcelSubmit(paramsJson,url);
...
function exportExcelHF(gridColumns, fileName, url, params, gridDatas, tableTitle) {
	var columns = [];
	for (var i = 0; i < gridColumns.length; i++) {
		var gridColumn = gridColumns[i];
		var type = gridColumn.dataIndex;
		var name = gridColumn.text;
		if (type != null && type != '') {
			columns.push({
				'type': type,
				'name': name
	...
	var exportExcelWin = Ext.create('Ext.window.Window', {
		...
		items: {
			...
			buttons: [{
				...
				handler: function() {
					var selectedColumns = [];
					var selectedGridColumns = exportExcelWin.down('grid').getSelectionModel().getSelection();
					if (selectedGridColumns.length > 0) {
						for (var i = 0; i < selectedGridColumns.length; i++) {
							var selectedGridColumn = selectedGridColumns[i];
							selectedColumns.push({
								'type': selectedGridColumn.get('type'),
								'name': selectedGridColumn.get('name')
							});
						}
						...
						titleField.value = JSON.stringify(selectedColumns);
						downForm.appendChild(titleField);

						var paramsField = document.createElement('input');
						paramsField.type = 'hidden';
						paramsField.name = 'params';
						paramsField.value = JSON.stringify(params);
						downForm.appendChild(paramsField);

						var fileNameField = document.createElement('input');
						fileNameField.type = 'hidden';
						fileNameField.name = 'fileName';
						fileNameField.value = fileName;
						downForm.appendChild(fileNameField);
						if (gridDatas.length > 0) {
							Ext.MessageBox.confirm('提示', '是否导出勾选的数据', function(obj) {
								if (obj == 'yes') {
									var datas = [];
									for (var i = 0; i < gridDatas.length; i++) {
										datas.push(gridDatas[i].raw);
									}
									...
									dataField.name = 'data';
									dataField.value = JSON.stringify(datas);
									downForm.appendChild(dataField);
							...
							Ext.MessageBox.confirm('提示', '是否导出所有符合过滤条件的数据', function(obj) {
								if (obj == 'yes') {
				...
				handler: function() {
					if (tableTitle != undefined) {
						Ext.Ajax.request({
							...
							success: function(response) {
								var result = Ext.decode(response.responseText);
								columnStore.loadData(result.dataList);
							}
						})
					} else {
						Ext.MessageBox.alert('提示', '该表暂未配置更多列');
