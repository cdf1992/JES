	...
	var customerGrid = new Ext.grid.GridPanel({
		...
		tbar : [{
			...
			handler : function(me) {
				...
				params['f.CUSTOMER_ID:eq'] = Ext.getCmp('customerNumP').getValue();
				params['f.CUSTOMER_CNAME:likeR'] = Ext.getCmp('customerNameP').getValue();
				params["f.CUSTOMER_TAXNO:eq"] =Ext.getCmp('customerTaxNumP').getValue();
				params['f.TAXPAYER_TYPE:eq'] =Ext.getCmp('taxpayerTypeP').getValue();
				customerStore.proxy.extraParams =params;
				customerStore.loadPage(1);
			...
			handler : function() {
				...
				if(records.length<=0){
					...
					var customer = records[0].data;
					var customerId = customer.customerId;
					var customerTaxno = customer.customerTaxno;
					var customerCname = customer.customerCname;
					var customerAddress = customer.customerAddress;
					if (customerAddress == undefined) {
						customerAddress = '';
					}
					var customerPhone = customer.customerPhone;
					if (customerPhone == undefined) {
						customerPhone = '';
					}
					var customerCbank = customer.customerCbank;
					if (customerCbank == undefined) {
						customerCbank = '';
					}
					var customerAccount = customer.customerAccount;
					if (customerAccount == undefined) {
						customerAccount = '';
					}
					var fapiaoType = customer.fapiaoType;
					//预开
					if("preOpen"==flag){
						myFind.query('textfield[name=billInfo.customerId]')[0].setValue(customerId);  //客户id
						myFind.query('textfield[name=billInfo.customerCname]')[0].setValue(customerCname);   //客户名称
						myFind.query('textfield[name=billInfo.customerTaxno]')[0].setValue(customerTaxno);  //客户纳税人识别号
						myFind.query('textfield[name=billInfo.customerName]')[0].setValue(customerCname);  //客户纳税人名称
						//客户地址电话
						myFind.query('textfield[name=billInfo.customerAddressandphone]')[0].setValue(customerAddress + customerPhone);
						//客户银行账号
						myFind.query('textfield[name=billInfo.customerBankandaccount]')[0].setValue(customerCbank + customerAccount);
						myFind.query('textfield[name=billInfo.fapiaoType]')[0].setValue(fapiaoType);  //发票类型
					}else{
						var $customerCnames = myFind.query('textfield[name=billInfo.customerCname]');
						if (!$customerCnames || $customerCnames.length == 0) {
							$customerCnames = myFind.query('textfield[name=customerCname]');
						}
						if (!$customerCnames || $customerCnames.length == 0) {
							$customerCnames = myFind.query('textfield[name=f.CUSTOMER_NAME:eq]');
						}
						if (!$customerCnames || $customerCnames.length == 0) {
							$customerCnames = myFind.query('textfield[id=customerName]');
						}
						if ($customerCnames && $customerCnames.length > 0) {
							var $customerCname = $customerCnames[0];
							$customerCname.setValue(customerCname);
						}

						var $customerIds = myFind.query('textfield[name=customerId]');
						if (!$customerIds || $customerIds.length == 0) {
							$customerIds = myFind.query('textfield[name=f.CUSTOMER_ID:eq]');
						}
						if (!$customerIds || $customerIds.length == 0) {
							$customerIds = myFind.query('textfield[id=customerId]');
						}
						if ($customerIds && $customerIds.length > 0) {
